...
slopes :: (Ord a, Fractional a) => Mode -> [Pair a] -> ([(Pair a,a)],[P2 a])
slopes m = foldl filter ([],[]) 
  where 
    filter (ns,cs) (p,q)  
      = case slope (Segment p q) of 
          Vertical -> (ns, select m : cs)
          Slope sl -> (((p,q), sl):ns, cs)
      where 
        select Upper  = if p `greaterY` q then p else q
