...
module KDTree (
         KDTree (..), 
         empty, depth, fromList, toList, 
         member, rangeQuery,  
         pprint, module Assocs
       ) where
...
data KDTree a 
  = Nil 
  | Leaf a
  -- Node Dimenson Value lower equal higher
...
fromList ps = build 1 ps 
  where 
    ...
    build _ []  = Nil
    build _ [x] = Leaf x
    build i xs  = Node i m (build j ls) (build j es) (build j hs)
      where 
...
member _ Nil      = False
member q (Leaf p) = p <==> q
member q (Node i m ls es hs) 
  = case compareIth i q m of
      LT -> q `member` ls
      EQ -> q `member` es
      GT -> q `member` hs


rangeQuery :: (Point p, Ord a, Num a) => KDTree (p a) -> (p a, p a) -> [p a]
