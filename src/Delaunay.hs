...
org q x@(i, r, _)
  = do e <- Q.getAttr q x
       return (if r==Rot0 then source e else target e)
dest q x@(i, r, _)      
  = do e <- Q.getAttr q x
       return (if r==Rot0 then target e else source e)

connect                  :: QEDS s a -> EdgeRef -> EdgeRef -> ST s EdgeRef
connect q a b             
  = do s <- dest q a; t <- org q b
...
delone q [s1,s2]          
  = do a <- makeEdge q (DelEdge {source=s1, target=s2})
       return (a, sym a)
...
delone q ss@(_:_:_:_:_)          
  = do (ldo, ldi) <- delone q ls
...
merge q base              
  = do l <- deleteEdges q base onext (sym base)
...
deleteEdges q base next b     
  = do e <- next q b
       ifM (isValid q base e) (try e) (return e)
 where 
   try e
    = do t <- next q e
         ifM (inCircleM (dest q base) (org q base) (dest q e) (dest q t))
            (do deleteEdge q e; try t) (return e)
